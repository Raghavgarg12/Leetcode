...
public:
    ...
    string convertToWords(int num) {
        ...
        if (num < 100) return belowHundred[num / 10] + (num % 10 ? " " + convertToWords(num % 10) : "");
        if (num < 1000) return convertToWords(num / 100) + " Hundred" + (num % 100 ? " " + convertToWords(num % 100) : "");
        if (num < 1000000) return convertToWords(num / 1000) + " Thousand" + (num % 1000 ? " " + convertToWords(num % 1000) : "");
        if (num < 1000000000) return convertToWords(num / 1000000) + " Million" + (num % 1000000 ? " " + convertToWords(num % 1000000) : "");
